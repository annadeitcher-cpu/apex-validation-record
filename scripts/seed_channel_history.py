...
CONVERSATION = [
    ...
    ("Marcus Webb (SolCon)", ":man:", "calendly link is out for the 22nd, they took the 10am slot"),
    ("Marcus Webb (SolCon)", ":man:", "curtis nam (security) is joining too apparently, sent him the encryption one-pager just in case"),
    ("Elena Restrepo (CSM)", ":woman:", "not mine yet right, just lurking"),
    ("Marcus Webb (SolCon)", ":man:", "yeah still TV, i'll tag you if/when"),

    # Short exchange after the technical validation call — Curtis's questions
    ("Marcus Webb (SolCon)", ":man:", "that ran long lol. curtis had like 20 min of questions on tokenization/encryption"),
    ("Elena Restrepo (CSM)", ":woman:", "ooh what specifically did curtis push back on? want to flag it if it's a pattern"),
    ("Marcus Webb (SolCon)", ":man:", "nothing major tbh, just wanted the details on how it all works, not really objections"),
    ("Marcus Webb (SolCon)", ":man:", "think it landed ok, he said he was satisfied by the end"),
    ("Marcus Webb (SolCon)", ":man:", "soc2 going out under nda today"),

    # Naomi's mapping comes back, a couple weeks later
    ("Marcus Webb (SolCon)", ":man:", "naomi sent the snowflake/teradata mapping over. mostly clean, one gap on the call center system"),
    ("Marcus Webb (SolCon)", ":man:", "not on their migration roadmap apparently lol, gonna need a connector for that one"),
    ("Marcus Webb (SolCon)", ":man:", "2nd call today, fine, nothing new really"),

    # Derek's departure surfaces informally
    ("Marcus Webb (SolCon)", ":man:", "did we know derek left meridian?? just found out from naomi"),
    ("Marcus Webb (SolCon)", ":man:", "someone named priya raman taking over, she's his old boss i think"),
    ("Marcus Webb (SolCon)", ":man:", "guess we'll find out more thursday"),
    ...
    ("Marcus Webb (SolCon)", ":man:", "tbh not really, still getting the new person up to speed"),
]
...
INTERLUDE = [
    ("Marcus Webb (SolCon)", ":man:", "priya's legal team signed off on the BAA amendment, that was the last blocker on our end"),
