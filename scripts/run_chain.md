...
7. **Get render context.**
   ...
   ```
   Prints the written Record payload, this version's change events, and deal metadata (account, ACV, AE/SolCon first names, days in stage, and how that compares to the Apex-wide average) — everything pass 4 needs, computed once, deterministically, rather than guessed at write time.
