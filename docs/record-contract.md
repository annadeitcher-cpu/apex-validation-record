...
```json
{
  ...
  "narrative": {
    ...
    "sc_loop_close": "Prose, addressed to the SolCon: did their prior resolutions hold up this version, did anything they handled get reopened. If nothing technical is blocking, say so explicitly — that's the feedback a SolCon otherwise never gets.",
  ...
  "objections": [{
    ...
    "confidence": "high | medium | low",
    "resolution_note": "SolCon walked through field-level encryption; customer accepted (or null if unresolved)",
    "raised_call": "uuid",
