...
| SolCon, Enterprise | Hands back to AE and never hears what happened |
...
| Current State doc | SolCon summary completes ~60% of the time; nothing systematic flows to post-sales |
...
**Product release velocity** (SF AE, Regional Sales Manager, MM SolCon). Same shape. Enablement lag is a content-supply problem. *Also folded in — see §3, product relevance.*
...
```json
{
  ...
  "objections": [{
    ...
    "resolution_note": "SolCon walked through field-level encryption; customer accepted",
    "raised_call": "uuid",
...
- AE, SolCon, and 2–4 customer-side people. At least one person in Calibre who never introduces themselves.
...
| **DM to AE or SolCon, 72h** | Private reminder naming the specific missed action item and its due date | Escalation is never a surprise |
...
- SolCon review time drops because the Record already reflects what happened on the call, extracted from the transcript — nothing to write from scratch.
...
- SolCon summary completion: 60% → 90%
...
1. **False-resolved objections.** Highest-probability failure. Detection: SolCon correction rate on the objection block. Mitigation: require customer acceptance evidence; default to `partially_resolved`.
...
| 4:00–7:00 | **The workflow** | Walk it as the AE experiences it. Then the SolCon, CS, manager. Emphasize: one object, four renderings. |
...
1. **Current state.** The Meridian deal channel as it exists today — AE, SolCon, CSM in it, and after the technical validation call, silence. That screenshot *is* the problem.
...
Batch off the warehouse, not per-call API into Gong. One extraction per new call on active opportunities. Re-runnable when prompts improve, without re-pulling from Gong.
...
That's the real risk, and it's why every hygiene mechanism is subtractive. SolCon review time drops because the Record already reflects the call, not because I added a reminder. The measure that matters is reminder-to-resolution rate, not opens. If that decays after week 3, the system isn't less work than the status quo and I'd rebuild the surface, not add nagging.

**"What would you build next?"**
The pattern library across Records: which objection-handling approaches precede advancement, which integration concerns recur by vertical, which competitor mentions correlate with stall. Framed as a shared SolCon asset, not an individual scorecard. It needs about a quarter of volume before the patterns are trustworthy — I wouldn't ship it on thin data.
