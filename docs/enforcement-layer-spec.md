...
|---|---|---|
| All technical objections resolved or accepted | SolCon | `objections` — none `open` or `resurfaced` |
| Economic buyer identified | AE | a stakeholder with `role_inference: economic_buyer`, confidence high or medium |
| Integration approach agreed | SolCon | every `integration_patterns` entry has `agreed: true` |
...
- SolCon items → the corresponding Record field changed state
...
**The line to use when challenged:** *"The private nudge comes first, so the escalation is never a surprise. It posts to the deal channel — the AE, the SolCon, the CSM — not a sales-wide channel. That's a working team, not an audience. A nudge with no escalation is a nudge with no teeth, and that's how the Confluence guide died."*
...
- On-time completion rate, per AE and per SolCon
