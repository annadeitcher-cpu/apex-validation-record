...
1. **No real acceptance on record.** Hearing the SolCon give a good answer and marking it resolved when the customer never actually reacted. If only the SolCon or AE spoke and there's no customer reaction on record, mark `partially_resolved`, not `resolved`.
...
- **integration_patterns**: dedupe. Set `agreed` (true only on real agreement, not just the SolCon proposing it), `confidence`, and `caveats` (or null).
...
Write in plain prose. No field labels, no verbatim evidence quotes — this is read by an AE and a SolCon on a phone, not audited like the rest of the record. The first time you name a prospect-side person, give their title (`"Priya Raman, VP of Data Platform"`); first name only after that, in this draft and in every later one. Internal Apex people (AE, SolCon) never get a title here — just the first name.
...
- **sc_narrative_candidate**: direct, from-this-call-only feedback to the SolCon — did anything they previously resolved get revisited or reopened this call, did a new technical concern land that's theirs to own, or is there genuinely nothing new on the technical side. This is pass 3's raw material for the accrued SolCon loop-close, not the final version (pass 3 sees whether something resolved earlier actually held up across the *whole* deal, not just this call).
