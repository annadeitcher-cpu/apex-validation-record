...
- **sc_loop_close**: the current accrued answer to "how did the SolCon's work hold up." Use the full cross-call picture, not just this call: if something the SolCon resolved earlier got reopened *this* version, say so plainly and directly to them — that's the single most important thing this field exists to carry. If nothing technical is blocking as of this version, say that explicitly; a SolCon who gets told "nothing's blocking" is getting real information, not a non-answer.
- **sc_actions**: current outstanding SolCon actions, if any — often empty, and an empty list is a fine, correct answer.
