...
**Inputs:** the output of `scripts/get_render_context.py --deal <name> --version <n>` — the Record payload, this version's change events, days-in-stage, AE and SC first names, and today's date.
...
Message-type header → Header → What's happened → Progress toward stage exit → What's blocking us → AE actions → SC actions → Need from [customer] → Worth raising
...
**[SC first name] — SC.** Numbered actions, each with a real date. If nothing technical is blocking, that's the final item: one sentence, no elaboration.
...
Mark the single blocking item with a 🔴 prefix, used consistently everywhere it appears (What's blocking us, the AE's or SC's action list, and Need from the customer, if it shows up in more than one place).
